...
async function fetchVideoAsBase64(videoUrl: string, timeout = 10000): Promise<string | null> {
    try {
        const response = await axios.get(videoUrl, {
            responseType: "arraybuffer", // Ensures we get binary data
            timeout, // Set timeout in milliseconds
...
async function handleBlobUrl(page: Page, blobUrl: string, outputPath: string): Promise<void> {
  try {
...
// };
 ...
 console.log(`✅ Saved video file for tweet ${tweetId} at ${videoFilePath}`);
} else {
 console.log(`❌ Could not find video URL for tweet ${tweetId}`);
